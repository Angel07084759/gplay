...
public class MainX01 extends AppCompatActivity implements TextWatcher, View.OnFocusChangeListener
{
    ...
    public void click(View view)
    {
        ...
        Log.d("DEB", "{Main.click(A)}" + ((startAddr == null) ? " NULL " :startAddr.toString()));
        new LocationTask(this, new LocationTask.LocationTaskResponse()
        {
            ...
            public void onProcessFinish(List<Address> addresses)
            {
                ...
                {
                    new LocationTask(MainX01.this, new LocationTask.LocationTaskResponse()
                    {
                        @Override
                        public void onProcessFinish(List<Address> addresses)
                        {
                            ...
                            Log.d("DEB", "{Main.click(C)}");
                        }
                    }).runTask();//Current user location
                    //return;
                }
                //addrStart = addresses.get(0);
                Log.d("DEB", "{Main.click(-1-)}" + (startAddr == null));//CRASHES
  ...
  /*                      if (ACTVdestAddr.getText().toString().trim().length() == 0)
                        ...
                        Log.d("DEB", "{Main.click(D)}");
                    }
                }).runTask(ACTVdestAddr.getText().toString().trim());



                Log.d("DEB", "{Main.click(B)}");
            }
...
//                    double latStart = addrStart.getLatitude();
//                    double lngStart = addrStart.getLongitude();
//                    double latDest = addrDest.getLatitude();
//                    double lngDest = addrDest.getLongitude();
//
...
//                    return;
//                }
//                Log.d("DEB", "{Main.click(2)}" + (addrStart == null));//CRASHES
///*                if (ACTVdestAddr.getText().toString().trim().length() == 0)
//                {
//                    double latStart = addrStart.getLatitude();
//                    double lngStart = addrStart.getLongitude();
            ...
            new LocationTask(this, new LocationTask.LocationTaskResponse()
            {
                ...
                public void onProcessFinish(List<Address> addresses)
                {
                    if (addresses != null && addresses.size() > 0)
                    {
                        ...
                        ArrayAdapter<String> adapter = new ArrayAdapter<String>(MainX01.this, android.R.layout.simple_dropdown_item_1line, addrs);
